...
public class AutoBidDAO {
    ...
    public static boolean saveAutoBid(String roomId, String username, long maxPrice) {
        synchronized (connection) {
            String sql = """
                    INSERT INTO AutoBid(roomId, username, maxPrice) VALUES(?, ?, ?) 
            ...
            try (PreparedStatement preparedStatement = connection.prepareStatement(sql)) {
                ...
                preparedStatement.setLong(4, maxPrice);
    ...
    public static List<AutoBidSetting> getAutoBidders(String roomId) {
        ...
        String sql = "SELECT username, maxPrice FROM AutoBid WHERE roomId = ?";
        synchronized (connection) {
            try (PreparedStatement preparedStatement = connection.prepareStatement(sql)) {
                preparedStatement.setString(1, roomId);
                try (ResultSet resultSet = preparedStatement.executeQuery()) {
                    while (resultSet.next()) {
                        list.add(new AutoBidSetting(roomId, resultSet.getString("username"), resultSet.getLong("maxPrice")));
                    }
                }
            } catch (SQLException e) {
